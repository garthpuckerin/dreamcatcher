...
export function useDreams() {
  ...
  const updateFragment = useCallback(
    async (id, updates) => {
      try {
        const { error } = await supabase.from('fragments').update(updates).eq('id', id)

        if (error) throw error

        // Find the dream_id to update its timestamp
        const fragment = dreams.flatMap(d => d.fragments).find(f => f.id === id)
  ...
  const updateTodo = useCallback(
    async (id, updates) => {
      try {
        ...
        // Find the dream_id to update its timestamp
        const todo = dreams.flatMap(d => d.todos).find(t => t.id === id)
  ...
  const trackDreamAccess = useCallback(async dreamId => {
    try {
      const { error } = await supabase.rpc('track_dream_access', { p_dream_id: dreamId })
