...
export class StorageManager {
    ...
    async getDreams(): Promise<Dream[]> {
        try {
            const dreams = await this.context.globalState.get<Dream[]>('dreamcatcher.dreams', []);
            return dreams;
        } catch (error) {
    ...
    async updateDream(id: string, updates: Partial<Dream>): Promise<void> {
        try {
            const dreams = await this.getDreams();
    ...
    async addFragment(dreamId: string, fragment: Fragment): Promise<void> {
        try {
            const dreams = await this.getDreams();
    ...
    async deleteFragment(dreamId: string, fragmentId: string): Promise<void> {
        try {
            const dreams = await this.getDreams();
            const dreamIndex = dreams.findIndex(d => d.id === dreamId);
    ...
    async getDreamsByTag(tag: string): Promise<Dream[]> {
        try {
            const dreams = await this.getDreams();
    ...
    async getDreamsByStatus(status: string): Promise<Dream[]> {
        try {
            const dreams = await this.getDreams();
    ...
    async getDreamsByBrand(brand: string): Promise<Dream[]> {
        try {
            const dreams = await this.getDreams();
    ...
    async getRecentDreams(limit: number = 10): Promise<Dream[]> {
        try {
            const dreams = await this.getDreams();
            return dreams
    ...
    async getDreamStatistics(): Promise<{
        totalDreams: number;
        totalFragments: number;
        dreamsByStatus: Record<string, number>;
        dreamsByBrand: Record<string, number>;
        ...
        try {
            ...
            const recentActivity = dreams.filter(dream => 
                new Date(dream.updated) > sevenDaysAgo
            ).length;
    ...
    async importDreams(jsonData: string): Promise<void> {
        try {
            const dreams = JSON.parse(jsonData) as Dream[];
            await this.context.globalState.update('dreamcatcher.dreams', dreams);
        } catch (error) {
